...
export default function APIKeysPage() {
  ...
  const fetchAPIKeys = async () => {
    try {
      const data = await api.get("http://109.237.64.130:8000/api/api-keys");
      setApiKeys(data);
    } catch (error) {
      toast({
        title: "Error",
  ...
  const createAPIKey = async () => {
    ...
    setIsCreating(true);
    try {
      const data = await api.post("http://109.237.64.130:8000/api/api-keys", {
        name: newKeyName,
        is_active: true,
      });
  ...
  const deleteAPIKey = async (id: number) => {
    try {
      const response = await api.delete(`http://109.237.64.130:8000/api/api-keys/${id}`);
  ...
  const toggleAPIKeyStatus = async (id: number, currentStatus: boolean) => {
    try {
      const response = await api.put(`http://109.237.64.130:8000/api/api-keys/${id}`, {
