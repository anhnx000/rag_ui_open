  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      const result: UploadResult = await api.post(
        `http://109.237.64.130:8000/api/knowledge-base/${params.id}/documents/upload`,
        formData
      );
  ...
  const startProcessing = async () => {
    ...
    try {
      ...
      const response = await api.post(
        `http://109.237.64.130:8000/api/knowledge-base/${params.id}/documents/process`,
        uploadResults
      );
  ...
  const checkProcessingStatus = async () => {
    ...
    try {
      ...
      const status: Record<string, TaskStatus> = await api.get(
        `http://109.237.64.130:8000/api/knowledge-base/${params.id}/documents/tasks?task_ids=${taskIds}`
      );
